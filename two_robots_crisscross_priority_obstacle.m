%% Two-Robot Collision Avoidance with a Static Obstacle and Dynamic Priority
%
% This script simulates two Dubins car models navigating to their respective
% goals. The key challenge is to simultaneously avoid collisions with each other
% and with a fixed circular obstacle in the environment.
%
% The control architecture is based on a safety-critical framework that uses
% Control Barrier Functions (CBFs) formulated within a single
% Quadratic Program (QP). This QP finds an optimal control action that
% respects all safety constraints at once.
%
% To handle complex interactions, the simulation also implements a dynamic
% priority system. The robot initially closer to its goal is given "priority,"
% which is enforced by a weighted cost function in the QP. This encourages
% the non-priority robot to yield, helping to resolve potential deadlocks.
...
if dist_to_goal_1 <= dist_to_goal_2
    priority_robot = 1;
    fprintf('Robot 1 has priority.\n');
else
    priority_robot = 2;
    fprintf('Robot 2 has priority.\n');
end

% --- Priority Logic Variables ---
priority_robot_reached_goal = false; % Flag to track if the priority robot has finished.
priority_weight = 100; % A high weight to penalize the priority robot's deviation.
...
while norm(states(1:2, :) - goal_states(1:2, :)) >= 5e-2 && time < max_simulation_time
    ...
    if priority_robot_reached_goal
        w1 = normal_weight; w2 = normal_weight;
    else
        ...
        if priority_robot == 1
            w1 = priority_weight; w2 = normal_weight;
        else
            w1 = normal_weight; w2 = priority_weight;
        end
    end
    ...
    kappa = 5;
    nominal_controller_1 = kappa*(atan2(goal_states(2,1) - states(2,1), goal_states(1,1) - states(1,1)) - states(3,1));
    nominal_controller_2 = kappa*(atan2(goal_states(2,2) - states(2,2), goal_states(1,2) - states(1,2)) - states(3,2));
    ...
    g_2 = [0, 0, 1]';
    f_x = [f_1; f_2];
    g_x_u1 = [g_1; zeros(3,1)];
    g_x_u2 = [zeros(3,1); g_2];
    
    %% Safety-Critical Controller (CBF Constraints)
    % This section defines all safety constraints (robot-obstacle and
    % robot-robot) that will be enforced by the QP. A High-Order CBF (HOCBF)
    % structure is used because the control input has a relative degree of 2
    % with respect to the robot's position.

    % -- Constraint 1: Robot 1 vs Obstacle --
    ...
    b2 = L_f_B_obs_2 + class_K_const*B_obs_2; % RHS of the QP constraint
    
    % -- Constraint 3: Robot 1 vs Robot 2 --
    e_r1_r2 = states(1:2, 1) - states(1:2, 2);
    h_r12 = norm(e_r1_r2)^2 - safe_distance^2; % Standard CBF for inter-robot safety
    L_f_h_r12 = 2*e_r1_r2'*(f_1(1:2) - f_2(1:2));
    L_g1_B_r12 = 2*e_r1_r2' * [-linear_velocity*sin(states(3,1)); linear_velocity*cos(states(3,1))];
    L_g2_B_r12 = -2*e_r1_r2' * [-linear_velocity*sin(states(3,2)); linear_velocity*cos(states(3,2))];
    b3 = L_f_h_r12 + class_K_const*h_r12; % RHS of the QP constraint
    ...
    A = [-L_g1_B_r12, -L_g2_B_r12;  % Row 1: Inter-robot constraint
         -L_g_B_obs_1, 0;             % Row 2: Robot 1 vs Obstacle
         0, -L_g_B_obs_2];            % Row 3: Robot 2 vs Obstacle
    b = [b3; b1; b2];
    ...
    %% Update States in a Separate Loop
...
if time < max_simulation_time
    fprintf('\nMission Complete! Both robots have reached their goals.\n');
    fprintf('Total simulation time: %.2f seconds.\n', time);
else
    fprintf('\nSimulation ended before goals were reached (timeout).\n');
    fprintf('Total simulation time: %.2f seconds.\n', time);
end
...
for plt_idx = 1:10:length(time_history) % Speed up plotting by skipping frames
    ...
    % Plot start and goal points for both robots.
    ...
    legend([p1, p2, p_obs], 'Robot 1', 'Robot 2', 'Obstacle', 'location', 'best');
